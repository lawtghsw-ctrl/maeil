  ...
  return (
    <Card className="p-5">
      <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm font-semibold text-slate-900">서류 체크리스트</div>
                ...
                {items.map((item) => (
                  <li key={item.id}>
                    <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs hover:bg-slate-50">
                      ...
                      />
                      <span>
                        <span className={checked[item.id] ? "text-slate-400 line-through" : "text-slate-900"}>
                          {item.no}. {item.label}
                        </span>
                        {item.note && <span className="block text-[11px] text-slate-400">{item.note}</span>}
                      </span>
                    </label>
                  </li>
                ))}
