            ...
            {loanRecords.map((row, i) => (
              <tr key={row.id} className="border-t border-slate-100">
                <td className="px-2 py-1">
                  <Select value={row.kind1} onChange={(e: ChangeEvent<HTMLSelectElement>) => updateRow(i, { kind1: e.target.value as LoanRecord["kind1"] })} className="h-7 sm:h-7 w-20 px-1.5 text-[11px]">
                ...
                <td className="px-2 py-1">
                  <Input className="h-7 sm:h-7 w-28 px-1.5 text-[11px]" value={row.kind2 ?? ""} onChange={(e: ChangeEvent<HTMLInputElement>) => updateRow(i, { kind2: e.target.value })} placeholder="예: 신용대출(100)" />
                </td>
                <td className="px-2 py-1">
                  <Input className="h-7 sm:h-7 w-24 px-1.5 text-[11px]" value={row.lender ?? ""} onChange={(e: ChangeEvent<HTMLInputElement>) => updateRow(i, { lender: e.target.value })} placeholder="예: 제이티저축은행" />
                </td>
                <td className="px-2 py-1">
                  <input
                    type="date"
                    className="h-7 sm:h-7 rounded-lg border border-slate-200 bg-white px-1.5 text-[11px] outline-none"
