...
export default function SchedulePage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-bold text-ink">일정관리</h1>
        <p className="mt-0.5 text-sm text-muted">법원기일 · 서류제출기한 · 상담예약</p>
      </div>
          ...
          {RANGE_FILTERS.map((r) => (
            <button
              ...
              className={`rounded-sm2 px-2.5 py-1 text-xs font-medium ${
                range === r ? "bg-navy text-white" : "bg-line text-muted"
              }`}
            >
              {r}
            </button>
      ...
      <div className="card divide-y divide-line p-0">
          ...
          <div key={s.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
            <span
              className={`shrink-0 rounded-sm2 px-1.5 py-0.5 text-[11px] font-semibold ${
                diffDays <= 3 ? "bg-danger-tint text-danger" : "bg-brand-pale text-brand"
              }`}
            >
              {ddayLabel(s.date)}
            </span>
            <span className="shrink-0 rounded-sm2 bg-line px-1.5 py-0.5 text-[11px] text-muted">
              {s.type}
            </span>
        ...
        {rows.length === 0 && (
          <div className="py-10 text-center text-sm text-muted">해당 조건의 일정이 없습니다.</div>
        )}
      </div>
    </div>
  );
